...
{
    /**
     * Entry IDs created for the element-link cases; distinct, so preloading can't be faked by
     * deduplication.
    */
    ...
    */
    private function countQueries(callable $fn): int
    {
        ...
        // Widened deliberately: yii\log\Logger::$messages is declared as a bare `array`, so static
        // analysis narrows it to an empty one here.
    ...
    */
    ...
    /**
     * The N+1 this suite exists to keep dead: entry/category/asset links were resolved with one
     * element query *each*, so a 100-item menu cost 100 queries to build.
    */
    ...
    /**
     * A cache hit re-checks visibility against the current rows, which is one read — and must
     * stay one read, of two columns, however big the menu is.
    */
    ...
    /**
     * Saving an item used to read its menu twice from the database — once for its custom field
     * definitions, once for its maxDepth — so a bulk edit cost two menu queries per item on top
     * of its own writes.
    */
