...
/**
 * Shared by entry/category/asset link types.
*/
class ElementLinkResolver implements PreloadingLinkTypeResolverInterface
{
    /**
     * How many IDs go into one preload query.
    */
    private const PRELOAD_CHUNK_SIZE = 500;

    /**
     * Elements gathered by {@see preload()}, as `[siteId][elementId] => element|null`.
     *
    ...
    */
    ...
    public function resolve(MenuBuilderItem $item): ResolvedLink
    {
        ...
        // array_key_exists(), not `??`: a preloaded *absence* is stored as null and must
        // short-circuit exactly like a hit, or an unavailable element would be re-queried once per
        // item — the N+1 this exists to remove, in its worst case.
    ...
    */
    public function preload(array $elementIds): void
    {
        ...
        // Every wanted ID is seeded as an absence *first*, so an ID the query doesn't come back
        // with stays recorded rather than falling through to a per-item query.
    ...
    /**
     * Availability is decided from the element's *status*, not its `enabled` flag: the query above
     * is scoped to the current site, and `Element::getStatus()` is the only one of the two that
     * accounts for per-site enabled state (`enabledForSite`) — an element enabled globally but
     * disabled for the site being rendered must not produce a link there.
     *
    ...
    */
    ...
    /**
     * Public + static for the same testability reason as {@see isPubliclyAvailable()} — this is
     * the whole fallback decision for an unavailable element, with no element or app access of its
     * own.
    */
    public static function fallbackFor(MenuBuilderItem $item): ResolvedLink
    {
        return match ($item->fallbackBehavior) {
            // Re-checked rather than trusted for the same reason as UrlLinkResolver's customUrl: a
            // fallback URL becomes the rendered href, and a stored value isn't guaranteed to have
